...
|   🤖 **ИИ-ассистент**   | короткие ответы по вашим записям (опционально, GigaChat) |
...
- 🤖 **ИИ-ассистент (опционально)** — краткие сводки и ответы по записям дневника
...
## 🤖 ИИ-ассистент (по желанию)

В приложении есть опциональный ИИ-ассистент на базе GigaChat.

**Что он умеет:**

- 📝 давать краткую сводку по записям за период;
- ❓ отвечать на вопросы по вашему дневнику наблюдений.

**Безопасность:**

- 🚫 не вставляйте в запросы реальные медицинские документы;
- 🩺 ответы ассистента носят **справочный характер** — это не диагноз и не назначение лечения;
- 🔑 ключ доступа хранится только в `.env` и не попадает в Git.

**Ограничения:**

- ⚙️ ассистент отключается параметром `GIGACHAT_AI_ENABLED=false`;
- 📉 объём контекста ограничен последними записями: `GIGACHAT_MAX_ENTRIES`;
- ⏱ таймаут запросов: `GIGACHAT_TIMEOUT_SECONDS`.

> [!IMPORTANT]
> ИИ-ассистент **не ставит диагнозы и не назначает лечение**.
...
Если нужен ИИ-ассистент — заполните блок переменных `GIGACHAT_*` (см. таблицу ниже).
...
> [!CAUTION]
> 🛡 Реальный `GIGACHAT_AUTH_KEY` вносите **только в `.env` на NAS**. Файлы `.env.example` и README не должны содержать рабочих ключей.
...
```bash
cp .env.example .env
# заполните .env: SECRET_KEY, ADMIN_PASSWORD и параметры GigaChat, если нужен ассистент
docker compose up -d --build
```

Если ассистент не нужен — установите в `.env` значение `GIGACHAT_AI_ENABLED=false`.
...
Результат используйте как значение `SECRET_KEY`.
...
| `GIGACHAT_AI_ENABLED`      | Включить ИИ-ассистента (`true`/`false`)   |
| `GIGACHAT_AUTH_KEY`        | Authorization-ключ GigaChat API (секрет!) |
| `GIGACHAT_MODEL`           | Модель GigaChat                           |
| `GIGACHAT_SCOPE`           | Scope API-ключа                           |
| `GIGACHAT_TIMEOUT_SECONDS` | Таймаут запросов к GigaChat, секунд       |
| `GIGACHAT_MAX_ENTRIES`     | Сколько записей отправлять ИИ в контекст  |
...
## 🔄 Обновление ИИ-ассистента

1. Обновите код (`git pull` или скопируйте файлы, включая `app/ai_assistant/`).
...
3. Убедитесь, что `/health` и `/ai/health` отвечают корректно.
...
- 🔑 ключи ИИ-ассистента храните только в `.env`, не коммитить;
- 🩺 ИИ-ответы — справочные, не медицинские рекомендации.
...
| —                      | ➕ ИИ-ассистент по записям (опционально, GigaChat) |
...
> Приложение не назначает лечение; автоматические оценки и ответы ИИ — справочные.
